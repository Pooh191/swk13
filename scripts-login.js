document.getElementById('loginForm').addEventListener('submit', function(event) {
    ...
    var users = [
        { username: "admin", password: "admin", prefix: "Mr.", firstName: "Admin", lastName: "User" },
    ...
    if (authenticatedUser) {
        ...
        localStorage.setItem('loggedInUser', JSON.stringify({ 
            prefix: authenticatedUser.prefix, 
        ...
        window.location.href = 'data.html'; // ไปยังหน้าหลัก
    } else {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    usernameInput.addEventListener('click', function () {
        usernameInput.classList.add('input-clicked');
        usernameInput.addEventListener('animationend', function () {
            usernameInput.classList.remove('input-clicked');
        }, { once: true });
    });
});